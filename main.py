...
app = Flask(__name__)
CORS(app, resources={r"/scrape_all": {"origins": "*"}})
...
def scrape_teatro_calima(url):
    ...
    for post in soup.find_all("div", class_="post-list-item"):
        ...
        if date_element:
            date_str = date_element.find_next_sibling(
                "span", class_="post-footer-value"
            ).text.strip()
...
def scrape_eticket(url):
    ...
    for event_link in soup.find_all("a", class_="URLCOMPRA botonevento"):
        ...
        date_time = unidecode(
            event_soup.find(
                "div", class_="font16 boldear600 borde_artista mayusculas_primera"
            ).text.strip()
        )
...
def scrape_all():
    ...
    teatro_calima_data = scrape_teatro_calima(
        "https://teatrocalima.com.co/inicio/entretenimiento/#page-content"
    )
